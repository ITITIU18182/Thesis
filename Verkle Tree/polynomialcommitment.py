...
H = bandersnatch.hash_to_point('H')
...
def prove(G_vec: PointVector, P: Point, x: Scalar, v: Scalar, a_vec: ScalarVector, r: Scalar) -> dict:
    ...
    U = bandersnatch.hash_to_point('U Fiat-Shamir hash', *statement)
    P_prm = P + v * U
    ...
    while splt > 1:
        ...
        l_j = bandersnatch.random_scalar()   # blinding factor
        r_j = bandersnatch.random_scalar()   # blinding factor
        ...
        L_vec.append(L_j)
        R_vec.append(R_j)
        u_j = bandersnatch.hash_to_scalar('LR Fiat-Shamir hash', *statement, L_j, R_j)
        u_vec.append(u_j)
    ...
    d = bandersnatch.random_scalar()   # blinding factor
    s = bandersnatch.random_scalar()   # blinding factor
    R = d * (G_prm[0] + b_prm[0] * U) + s * H
    c = bandersnatch.hash_to_scalar('ZKopen Fiat-Shamir hash', Q, R)
    z1 = a_prm[0] * c + d
    z2 = c * r_prm + s
    zkopen = [R, z1, z2]
...
def verify(proof: dict) -> bool:
    ...
    for (L_j, R_j) in zip(L_vec, R_vec):
        u_j = bandersnatch.hash_to_scalar('LR Fiat-Shamir hash', *proof['state'], L_j, R_j)
        u_vec.append(u_j)
    ...
    # build P_prm and Q
    U = bandersnatch.hash_to_point('U Fiat-Shamir hash', *proof['state'])
...
if __name__ == '__main__':
    ...
    x = bandersnatch.random_scalar()
    a_vec = ScalarVector([bandersnatch.random_scalar() for i in range(4)])   # polynomial coefficients
    v = poly_eval(x, a_vec)
    G_vec = PointVector([bandersnatch.random_point() for i in range(len(a_vec))])
